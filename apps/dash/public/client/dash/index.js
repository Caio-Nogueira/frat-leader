...
const proxyToSandbox = async (request, env2, path) => {
  const url = new URL(request.url);
  const targetUrl = `${SANDBOX_BASE_URL}${path}${url.search}`;
  const body = hasRequestBody(request.method) ? await request.arrayBuffer() : void 0;
  return env2.SANDBOX_API.fetch(
...
const index = {
  async fetch(request, env2) {
    ...
    if (request.method === "GET" && url.pathname === "/api/health") {
      return proxyToSandbox(request, env2, "/");
    }
    if (request.method === "POST" && url.pathname === "/api/sessions") {
      return proxyToSandbox(request, env2, "/sessions");
    }
    if (request.method === "GET") {
      const sessionMatch = url.pathname.match(SESSION_ROUTE);
      if (sessionMatch?.[1]) {
        return proxyToSandbox(request, env2, `/sessions/${decodeURIComponent(sessionMatch[1])}`);
      }
      const sessionHealthMatch = url.pathname.match(SESSION_HEALTH_ROUTE);
      if (sessionHealthMatch?.[1]) {
        return proxyToSandbox(request, env2, `/sessions/${decodeURIComponent(sessionHealthMatch[1])}/health`);
      }
    }
    ...
    if (sessionOpencodeMatch?.[1]) {
      ...
      return proxyToSandbox(request, env2, `/sessions/${sessionId}/opencode${opencodePath}`);
    }
    if (!env2.ASSETS) {
